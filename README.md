本库展示了一系列神经算子(Neural Operator)的实现，用于高效求解PDE。所涉及的神经算子类型包括：

- FNO（傅里叶神经算子）：基于Fourier变换的神经算子（1D、2D、3D）

- TFNO（张量化FNO）：带有Tucker分解的张量化版本（1D、2D、3D）
...
请修改 config 文件夹中的 wandb_api_key.txt 文件，将 WandB 的 API Key 填写进去，并修改script/login_wandb.py中的 API Key 路径。如果您尚未创建过 WandB 账号，请先[注册](https://wandb.ai)。
